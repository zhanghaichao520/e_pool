...
public class Const {
	public static LatLng mLastLocationData = null;
	public static LatLng mStartLocationData = null;
	public static LatLng mEndLocationData = null;
	...
	public static String team = "我们是华东交通大学的一支科创团队,一直致力于做一些创新创业的项目，也得到了学校等单位的支持，2016年获得江西省创新创业立项。我们都是一群践行学为我所用的大学生，在创新创业的浪涛中激流勇进，恳请各位领导评委进行批评指正，也希望能给予我们更多的支持。";
	public static String use = "\n1、注册：根据提示输入您的信息（确保正确，姓名一旦确定无法修改，请谨慎填写），点击确定按钮完成注册，系统会自动给注册时候的邮箱发送激活邮件，请先激活账户再登录。"
				+"\n2、登录：账户激活之后，在软件的登录界面输入用户名和正确的密码进行登录，也可以使用游客身份登录，但是游客身份不可以使用叫车功能。"
				+"\n3、拼车：①进入app主界面后在基础地图模块长按地图选择您的出发点，②起点设定完毕后点击出发按钮进入poi搜索界面，输入宁的目的城市以及目的地的关键词，点击搜索，系统会自动帮您标注包含您关键词的地方，点击标号选定目的地；③进入乘客信息登记页面，输入乘车人数、愿意等待的最长时间、以及想支付的小费（支付小费越多拼车越快哦），以及是否愿意拼车，点击去拼车，此时系统将会为您匹配最近最合适的车辆。并将司机的姓名、车牌号、手机等信息反馈给您，并且会告诉您所需支付的费用。"
				+"\n4、个人中心：显示您的基本信息，请确保完善与真实，这将用于您与车主取得联系。点击每项信息后面的修改按钮，在弹出框输入修改的内容，即可完成修改，我们将根据您违约次数评定信誉等级，这将决定您以后拼车时候的优先级，请不要违约。"
				+"\n5、修改密码：根据提示输入新旧密码进行修改。"
				+"\n6、反馈：写出您的建议及联系方式（选填），我们将积极接受一些改进建议"
				+"\n7、注销：返回登录界面。"
				+"\n8、退出：退出app。";
}
